...
{
    class CNAEBox : TextBox, INotifyPropertyChanged
    {
        ...
        {
            if (Text.Length != 7 && !string.IsNullOrWhiteSpace(Text)) MessageBox.Show("CNAE inválido");
            else
            {
                Text = Text.ParseToCNAE();
        ...
        {
            if (!ContemSoNumeros(e.Text) || Text.Length>=7)
            {
                e.Handled = true;
                return;
            }
